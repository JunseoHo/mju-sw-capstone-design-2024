...
from trainer.unet3plus.scripts import deeplab_model
import tensorflow as tf

images_dir = '../data/champions_league/dataset_pole/images/'  # 전체 이미지가 저장되어 있는 디렉토리의 상대 주소
json_path = '../data/champions_league/dataset_pole/instances_default.json'  # COCO Json 파일의 상대 주소
checkpoint_path = "../checkpoints/PoledeeplabV3plus_ckpt.weights.h5"  # 체크 포인트의 저장 위치
# export_path = "../checkpoints/unet3plus_export.weights.h5"  # 학습된 모델의 저장 위치
export_path = "../checkpoints/PoledeeplabV3plus_export.weights.h5"
input_size = (512, 512)  # 초기128/ 모델의 입력 크기 : OOM 발생 시 줄여주세요.
epoch = 300  # 에폭의 횟수
train_batch_size = 100  # 초기5/ 훈련용 데이터 미니배치당 크기 : OOM 발생 시 줄여주세요.
valid_batch_size = 100  # 초기5/ 검증용 데이터 미니배치당 크기 : OOM 발생 시 줄여주세요.
...
model = DeeplabV3Plus((input_size[0],input_size[1],3),num_classes);
    # unet3plus((input_size[0], input_size[1], 3), num_classes))
...
# model.fit(train_batches, validation_data=valid_batches, epochs=epoch)
